...
function Fail([string]$Message) {
    throw "P47K.2 install aborted: $Message"
}
...
Write-Host "P47K.2: verify payload hashes"
...
$tempRoot = Join-Path $env:TEMP ("cripta-p47k2-overlay-" + [guid]::NewGuid().ToString("N"))
$backupRoot = $null
...
try {
    ...
    Write-Host "P47K.2: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47K.2: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "P47K.2: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "P47K.2: precheck green; apply real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47K_2_" + $stamp)
    foreach ($file in Get-ChildItem -LiteralPath $payloadRoot -Recurse -File) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47K.2 installed successfully."
