...
function Fail([string]$Message) {
    throw "P47M.1 install aborted: $Message"
}
...
Write-Host "P47M.1: verify payload hashes"
$expectedHashes = @{}
foreach ($line in Get-Content -LiteralPath $hashFile) {
    ...
    if ([string]::IsNullOrWhiteSpace($relative)) { Fail "empty payload path in SHA256SUMS" }
    if ($relative.StartsWith("payload\", [System.StringComparison]::OrdinalIgnoreCase)) {
        Fail "invalid SHA256SUMS path; paths must be relative to payloadRoot: $relative"
    }
    if ([System.IO.Path]::IsPathRooted($relative) -or $relative.Contains("..")) {
        Fail "unsafe SHA256SUMS path: $relative"
    }
    if ($expectedHashes.ContainsKey($relative)) { Fail "duplicate SHA256SUMS path: $relative" }
    $expectedHashes[$relative] = $expected
}

$payloadFiles = Get-ChildItem -LiteralPath $payloadRoot -Recurse -File
if ($payloadFiles.Count -ne $expectedHashes.Count) {
    Fail "payload/hash file count mismatch: payload=$($payloadFiles.Count) hashes=$($expectedHashes.Count)"
}
foreach ($fileInfo in $payloadFiles) {
    $relative = $fileInfo.FullName.Substring($payloadRoot.Length).TrimStart("\")
    if (-not $expectedHashes.ContainsKey($relative)) { Fail "payload file missing from SHA256SUMS: $relative" }
    $actual = (Get-FileHash -Algorithm SHA256 -LiteralPath $fileInfo.FullName).Hash.ToLowerInvariant()
    if ($actual -ne $expectedHashes[$relative]) { Fail "payload SHA256 mismatch: $relative" }
}
foreach ($relative in $expectedHashes.Keys) {
    if (-not (Test-Path -LiteralPath (Join-Path $payloadRoot $relative))) {
        Fail "SHA256SUMS references missing payload file: $relative"
    }
}

$tempRoot = Join-Path $env:TEMP ("cripta-p47m1-overlay-" + [guid]::NewGuid().ToString("N"))
$backupRoot = $null
...
try {
    ...
    Write-Host "P47M.1: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47M.1: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    Write-Host "P47M.1: precheck green; apply real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47M_1_" + $stamp)
    foreach ($file in $payloadFiles) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47M.1 installed successfully."
