...
function Get-OrderbookPlan([string]$Symbol, [string]$CacheDir) {
    ...
    $total = [int](($tradeDay - $evaluationStartDay).TotalDays) + 1
    $processed = 0
    $started = Get-Date
    ...
    while ($day -le $tradeDay) {
        ...
        $now = Get-Date
        if (
            ($now - $lastHeartbeat).TotalSeconds -ge $HeartbeatSeconds -or
            $processed -eq $total
        ) {
            $elapsed = $now - $started
            $percent = if ($total -gt 0) { 100.0 * $processed / $total } else { 100.0 }
            $etaSeconds = if ($processed -gt 0) {
                [Math]::Max(0.0, ($total - $processed) * $elapsed.TotalSeconds / $processed)
            }
            else {
                0.0
            }
            ...
            Write-Host (
                "[heartbeat] stage=orderbook-preflight symbol={0} processed={1}/{2} " +
                "({3:N1}%) elapsed={4:hh\:mm\:ss} ETA~{5:hh\:mm\:ss}"
            ) -f @($Symbol, $processed, $total, $percent, $elapsed, $eta)
            $lastHeartbeat = $now
        }
        $day = $day.AddDays(1)
    }
