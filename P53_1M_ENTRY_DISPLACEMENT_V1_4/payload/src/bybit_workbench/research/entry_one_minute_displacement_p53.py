...
P53_VERSION = "P53_1M_ENTRY_DISPLACEMENT_V1_4"
...
class P53Config:
    ...
    def __post_init__(self) -> None:
        if self.expected_signals != EXPECTED_SIGNALS:
            raise ValueError("P53 V1.4 is frozen to the 1063 Entry V1 ALL9 cohort")
        if self.horizon_hours != DEFAULT_HORIZON_HOURS:
            raise ValueError("P53 V1.4 deeper-entry availability horizon is frozen to 3 hours")
...
def _load_one_minute_dataset(
    *,
    ...
    if initial_seed_price <= 0:
        raise ValueError(f"invalid causal initial 1m seed for {symbol}: {initial_seed_price}")
    ...
    return tuple(all_candles), {
        ...
        "candle_semantics": "previous_close_continuous_open_flat_zero_volume",
        "initial_seed_source": "frozen_5m_first_open_causal",
    ...
    *,
    ...
    for symbol_index, source in enumerate(sources, start=1):
        ...
        one_minute, one_provenance = _load_one_minute_dataset(
            ...
            cache_dir=cache_dir,
            heartbeat=heartbeat,
            initial_seed_price=five[0].open,
        )
