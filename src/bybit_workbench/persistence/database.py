...
_MIGRATION_8 = """
CREATE TABLE IF NOT EXISTS entry_bot_signals (
    signal_id TEXT PRIMARY KEY,
    strategy_id TEXT NOT NULL,
    strategy_version TEXT NOT NULL,
    symbol TEXT NOT NULL,
    direction TEXT NOT NULL CHECK(direction IN ('Long', 'Short')),
    candidate_bar_at TEXT NOT NULL,
    touch_at TEXT NOT NULL,
    entry_price TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    created_at TEXT NOT NULL
);
CREATE INDEX IF NOT EXISTS ix_entry_bot_signals_symbol_time
ON entry_bot_signals(symbol, touch_at);

CREATE TABLE IF NOT EXISTS position_handoffs (
    handoff_id TEXT PRIMARY KEY,
    source_signal_id TEXT NOT NULL REFERENCES entry_bot_signals(signal_id),
    strategy_id TEXT NOT NULL,
    strategy_version TEXT NOT NULL,
    symbol TEXT NOT NULL,
    ...
    state TEXT NOT NULL CHECK(state IN ('OPEN', 'CLAIMED', 'CLOSED')),
    claimed_by TEXT,
    claimed_at TEXT,
    payload_json TEXT NOT NULL,
    created_at TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_position_handoffs_state_time
ON position_handoffs(state, created_at);
CREATE INDEX IF NOT EXISTS ix_position_handoffs_symbol_state
ON position_handoffs(symbol, state, created_at);
"""


_MIGRATION_9 = """
CREATE TABLE IF NOT EXISTS entry_bot_candidate_events (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    event_id TEXT NOT NULL UNIQUE,
    occurred_at TEXT NOT NULL,
    symbol TEXT NOT NULL,
    ...
    candidate_bar_at TEXT,
    entry_price TEXT,
    last_price TEXT,
    distance_pct TEXT,
    flow_state TEXT NOT NULL,
    oi_state TEXT NOT NULL,
    reason TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    created_at TEXT NOT NULL
);
CREATE INDEX IF NOT EXISTS ix_entry_bot_candidate_events_symbol_time
ON entry_bot_candidate_events(symbol, occurred_at, id);
CREATE INDEX IF NOT EXISTS ix_entry_bot_candidate_events_type_time
ON entry_bot_candidate_events(event_type, occurred_at, id);
CREATE INDEX IF NOT EXISTS ix_entry_bot_candidate_events_candidate
ON entry_bot_candidate_events(candidate_id, occurred_at, id);
"""
