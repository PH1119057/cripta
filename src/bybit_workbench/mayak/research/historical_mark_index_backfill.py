...
from bybit_workbench.research.entry_basis_v6 import BasisResearchConfig, _download_price_klines

VERSION = "mayak-mark-index-source-backfill-v1"
SOURCE_SEMANTICS = "Bybit V5 linear mark/index price 5m; candle available at open plus 5m"
...
def _window(baseline: Path) -> tuple[list[Any], datetime, datetime]:
    ...
    end = datetime.fromtimestamp(max(item.touch_epoch for item in signals), UTC)
...
def run(
    *,
    ...
    def one(symbol: str) -> dict[str, Any]:
        ...
        mark_path = symbol_dir / "mark_price_5m.csv"
        index_path = symbol_dir / "index_price_5m.csv"
        mark = _download_price_klines(
            mark_path,
            config=config,
        ...
        if not mark or not index:
            raise RuntimeError(f"empty mark/index result for {symbol}")
        result = {
            "symbol": symbol,
            "mark_rows": len(mark),
        ...
        print(
            "MARK_INDEX_DONE " + json.dumps(result, ensure_ascii=False, sort_keys=True),
            flush=True,
        )
        return result
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Exact public 5m mark/index source backfill")
    ...
    total_rows = sum(
        int(item.get("mark_rows", 0)) + int(item.get("index_rows", 0))
        for item in manifest.get("files", [])
    )
    print(
        f"MAYAK_MARK_INDEX_BACKFILL=PASS symbols={len(symbols)} rows={total_rows}",
        flush=True,
    )
    return 0
