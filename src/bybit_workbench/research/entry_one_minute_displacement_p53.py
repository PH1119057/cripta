...
P53_VERSION = "P53_1M_ENTRY_DISPLACEMENT_V1_5"
...
class P53Config:
    ...
    check_deeper_availability: bool = True

    def __post_init__(self) -> None:
        if self.expected_signals != EXPECTED_SIGNALS:
            raise ValueError("P53 V1.5 is frozen to the 1063 Entry V1 ALL9 cohort")
        if self.horizon_hours != DEFAULT_HORIZON_HOURS:
            raise ValueError("P53 V1.5 deeper-entry availability horizon is frozen to 3 hours")
    ...
    *,
    ...
    if cache_path.is_file() and meta_path.is_file() and archive_path.is_file():
        ...
        if compatible:
            ...
            seed_compatible = (
                seed_price is None
                or (bool(candles) and candles[0].open == seed_price)
            )
            if seed_compatible:
...
def _boundary_seed_from_frozen_five(
    frozen_five_minute: tuple[Candle, ...],
    first_archive_day: date,
) -> tuple[datetime, Decimal]:
    boundary_at = datetime(
        first_archive_day.year,
        first_archive_day.month,
        first_archive_day.day,
        tzinfo=UTC,
    )
    matches = [item for item in frozen_five_minute if item.opened_at == boundary_at]
    if len(matches) != 1:
        raise ValueError(
            "frozen 5m dataset does not contain exactly one candle at public-trade "
            f"archive boundary {boundary_at.isoformat()}; matches={len(matches)}"
        )
    seed_price = matches[0].open
    if seed_price <= 0:
        raise ValueError(
            f"invalid causal 1m boundary seed for {boundary_at.isoformat()}: {seed_price}"
        )
    return boundary_at, seed_price
...
def _load_one_minute_dataset(
    *,
    ...
    if not days:
        raise ValueError(f"no public-trade archive days found for {symbol}")
    initial_seed_opened_at, initial_seed_price = _boundary_seed_from_frozen_five(
        frozen_five_minute,
        days[0][0],
    )
    ...
    previous_close: Decimal | None = initial_seed_price
    total = len(days)
    ...
    return tuple(all_candles), {
        ...
        "candle_semantics": "previous_close_continuous_open_flat_zero_volume",
        "initial_seed_source": "frozen_5m_open_at_first_public_trade_archive_boundary",
        "initial_seed_opened_at": initial_seed_opened_at.isoformat(),
        "initial_seed_price": initial_seed_price,
...
def validate_five_minute_equivalence(
    ...
    *,
    ...
    compared = 0
    price_mismatches: list[dict[str, str]] = []
    ...
    for candle in five_minute:
        ...
        if values[:4] != expected[:4]:
            ...
            if len(price_mismatches) >= 10:
                break
        elif values[4] != expected[4] and len(volume_mismatches) < 10:
            volume_mismatches.append(
                {
                    "opened_at": candle.opened_at.isoformat(),
                    "derived_volume": str(values[4]),
                    "bybit_5m_volume": str(expected[4]),
                }
            )
    if compared <= 0:
    ...
    if price_mismatches:
        raise ValueError(
            "trade-derived 1m OHLC does not reproduce frozen Bybit 5m OHLC; "
            f"first mismatches={price_mismatches}"
        )
    if strict_volume and volume_mismatches:
        raise ValueError(
            "trade-derived 1m OHLCV volume does not reproduce frozen Bybit 5m volume; "
            f"first mismatches={volume_mismatches}"
        )
    return {
        "compared_5m_candles": compared,
        "ohlcv_mismatches": len(volume_mismatches),
        "ohlc_mismatches": 0,
        "volume_mismatch_examples": volume_mismatches,
        "volume_check_mode": "strict" if strict_volume else "warning_only",
    }
...
def run_p53(
    ...
    *,
    ...
    for symbol_index, source in enumerate(sources, start=1):
        ...
        if not five:
            raise ValueError(f"frozen 5m dataset is empty for {symbol}")
        one_minute, one_provenance = _load_one_minute_dataset(
            dataset_dir=source.dataset_dir,
            symbol=symbol,
            cache_dir=cache_dir,
            heartbeat=heartbeat,
            frozen_five_minute=five,
        )
        equivalence = validate_five_minute_equivalence(
            one_minute,
            five,
            strict_volume=config.check_deeper_availability,
        )
        ...
        if config.check_deeper_availability:
            ...
            signal_map = {
                (item.direction, item.touch_at.isoformat()): item
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--skip-deeper-availability", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = P53Config(
        ...
        check_deeper_availability=not args.skip_deeper_availability,
    )
