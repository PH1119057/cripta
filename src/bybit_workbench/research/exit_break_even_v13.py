...
class PathSeries:
    ...
    missing_archive_days: tuple[str, ...] = ()
    ...
    @property
    def complete_through(self) -> datetime:
        # Compatibility fallback for synthetic/unit-test PathSeries constructed
        # without archive coverage metadata.
        return self.coverage_until or self.available_until


class TradeDayCache:
    """Small LRU cache for decompressed public-trade days.

    P45 previously decompressed the same .csv.gz day again for every signal whose
    72h path touched that day. Signals are chronological, so a bounded LRU keeps the
    hot 4-6 days while avoiding an unbounded all-90-days memory footprint.
    """
    ...
    def __init__(
        ...
        *,
        max_days: int = 6,
        loader: Callable[[Path], TradeDay] = _load_trade_day,
    ) -> None:
        if max_days <= 0:
            raise ValueError("max_days must be positive")
        self.max_days = max_days
        self._loader = loader
        self._items: OrderedDict[Path, TradeDay] = OrderedDict()
        self.hits = 0
        self.misses = 0
    ...
    def get(self, path: Path) -> TradeDay:
        ...
        self._items[path] = loaded
        self._items.move_to_end(path)
        while len(self._items) > self.max_days:
...
class ProgressReporter:
    ...
    def emit(
        ...
        *,
        ...
        print(
            f"[P46] stage={stage} processed={processed}/{total} "
...
class BaselinePathMetrics:
    ...
    last_tick_gap_72h_seconds: float | None
    complete_6h: bool
    complete_12h: bool
    complete_24h: bool
    complete_48h: bool
...
def _dates_for_horizon(start: datetime, hours: int) -> tuple[str, ...]:
    ...
    current = start.date()
    # Archives are complete UTC calendar days. The interval is [start, end), so an
    # endpoint exactly at 00:00 does not require the next day's archive.
...
def _day_start(day: str) -> datetime:
    return datetime.combine(datetime.fromisoformat(day).date(), dt_time.min, UTC)
...
def build_path_series(
    ...
    *,
    ...
    if len(continuous_days) == len(required_days):
        ...
        coverage_until = min(requested_end, _day_start(required_days[len(continuous_days)]))

    day_cache = cache or TradeDayCache(max_days=max(1, len(continuous_days)))
    start_ts = signal.touch_at.timestamp()
    end_ts = requested_end.timestamp()
    timestamps_list: list[float] = []
    moves_list: list[float] = []
    for day in continuous_days:
        tape = day_cache.get(archive_by_day[day])
        start_index = bisect.bisect_left(tape.timestamps, start_ts)
        end_index = bisect.bisect_right(tape.timestamps, end_ts)
        for index in range(start_index, end_index):
            timestamps_list.append(tape.timestamps[index])
            moves_list.append(
    ...
    timestamps = tuple(timestamps_list)
...
def _is_horizon_complete(path: PathSeries, hours: int) -> bool:
    ...
    return path.complete_through >= required_until
...
def baseline_metrics(path: PathSeries, *, initial_stop_pct: float) -> BaselinePathMetrics:
    ...
    return BaselinePathMetrics(
        ...
        available_until=path.available_until,
        coverage_until=path.complete_through,
        missing_archive_days=";".join(path.missing_archive_days),
        last_tick_gap_72h_seconds=max(
            0.0,
            (
                path.signal.touch_at
                + timedelta(hours=72)
...
def _write_summary_md(
    ...
    *,
    ...
    lines = [
        "# P46 Exit Path Audit — corrected P45 replay",
        "",
        (
            "Research-only. Entry V1 is frozen. This is a corrected replay of the "
            "P45 experiment and changes no live trading logic."
        ),
        "",
        (
        ...
        (
            ...
            "late-window signals may be right-censored when complete daily archives "
            "do not cover the requested horizon."
        ),
        (
...
def run_research(
    ...
    *,
    ...
    total_signals_all = sum(len(items) for items in signals_by_symbol.values())
    processed_signals = 0
    progress.emit(
        "path-build",
        processed=0,
        total=total_signals_all,
        force=True,
        detail="starting exact trade paths",
    )
    ...
    for source in sources:
        ...
        symbol_paths_list: list[PathSeries] = []
        for signal in signals:
            symbol_paths_list.append(
                build_path_series(
                    signal,
                    archive_by_day,
        ...
        symbol_paths = tuple(symbol_paths_list)
    ...
    for activation_r in config.activation_r_values:
        for be_buffer_bps in config.be_buffer_bps_values:
            ...
            policy_progress.emit(
                "policy-grid",
                processed=processed_policies,
                total=total_policies,
                detail=f"activation={activation_r:.2f}R buffer={be_buffer_bps:.1f}bps",
            )

    _write_csv(output_dir / "baseline_paths.csv", baseline_rows)
    _write_csv(output_dir / "break_even_policy_grid.csv", policy_rows)
    summary = {
        "architecture": "p46_exit_path_engine_audit_v1",
        "research_only": True,
        "entry_frozen": True,
        "initial_stop_pct": config.initial_stop_pct,
        "horizon_hours": config.horizon_hours,
        ...
        "notes": [
            (
                "Break-even buffers are research scenarios, not claimed "
                "account-specific fee values."
            ),
            (
            ...
            (
                "Horizon completeness is based on contiguous complete daily archive "
                "coverage, not on whether a trade happened within two seconds of the "
                "exact horizon boundary."
            ),
            (
                "Missing archive days terminate a path at the first gap; later days are "
                "not stitched across that gap."
            ),
            (
                "No trailing, partial TP, leverage, sizing, or live execution logic "
                "is changed by P46."
            ),
        ],
    }
...
def _default_output_dir(root: Path) -> Path:
    ...
    return root / "reports" / "exit_path_audit_v1" / f"UNI_LINK_{stamp}"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="P46 Exit Path Audit: corrected coverage + cached P45 replay"
    )
    ...
    print(f"P46 sources: {', '.join(source.symbol for source in sources)}")
    total_signals = sum(
        int(item["signals"]) for item in summary["baseline_by_symbol"].values()
    )
    print(f"P46 signals: {total_signals}")
    print(f"P46 policy grid rows: {summary['policy_grid_rows']}")
