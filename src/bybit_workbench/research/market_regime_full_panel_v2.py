...
def _dataset_dir_from_p30_comparison(
    ...
    *,
    ...
    if not _metadata_matches_period(comparison_path, start=start, end=end):
        return None
    try:
        payload = cast(
            dict[str, Any],
            json.loads(comparison_path.read_text(encoding="utf-8-sig")),
        )
    except (OSError, UnicodeError, json.JSONDecodeError, TypeError):
        return None
    ...
    if not isinstance(raw_dataset_dir, str) or not raw_dataset_dir.strip():
        return None
    dataset_dir = Path(raw_dataset_dir.strip())
    if not dataset_dir.is_absolute():
        dataset_dir = root / dataset_dir
    return dataset_dir


def resolve_frozen_5m_path(
    root: Path,
    *,
    symbol: str,
    start: datetime,
    ...
    primary = asset_root / "p30" / "dataset" / "trade_5m.csv"
    if primary.is_file():
        return primary, "cross_asset_p30"

    # P30 deliberately supports --dataset-dir separate from --output-dir.
    # Therefore the canonical report directory and the heavy frozen dataset
    # are not necessarily siblings.  The P30 comparison.json stores the exact
    # dataset_dir used for the run; follow that provenance instead of guessing.
    materialized_comparison = asset_root / "p30" / "comparison.json"
    dataset_dir = _dataset_dir_from_p30_comparison(
    ...
    if dataset_dir is not None:
        candidate = dataset_dir / "trade_5m.csv"
        if candidate.is_file():
            return candidate, "p30_comparison_dataset_dir"
        ...
        if isinstance(sources, dict):
            ...
            if isinstance(p30_source, str) and p30_source:
                ...
                if dataset_dir is not None:
                    candidate = dataset_dir / "trade_5m.csv"
                    if candidate.is_file():
                        return candidate, "legacy_materialization_p30_report"
    ...
    if legacy_base.is_dir():
        ...
        for comparison_path in comparison_paths:
            ...
            candidate = dataset_dir / "trade_5m.csv"
            if candidate.is_file():
                return candidate, "legacy_v3_comparison_search"
...
def run_analysis(
    *,
    ...
    for symbol in DEFAULT_SYMBOLS:
        asset_root = validation_root(root, symbol, start, end)
        price_path, price_source = resolve_frozen_5m_path(
            root,
            symbol=symbol,
            start=start,
            end=end,
        ...
        if not price_path.is_file():
            raise FileNotFoundError(
                f"missing frozen 5m data for {symbol}; checked cross-asset and legacy v3: "
                f"{price_path}"
            )
        ...
        source_paths.append((symbol, f"trade_5m:{price_source}", price_path))
        source_paths.append((symbol, "p40_core", signal_path))
        print(f"  OK {symbol}: trade_5m [{price_source}] + P40 core")
    ...
    summary: dict[str, Any] = {
        ...
        "data": {
            "network_used": False,
            "price_source": "resolved frozen local trade_5m.csv (cross-asset P30 or legacy v3)",
