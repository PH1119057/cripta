    ...
    for number in _action_keys(action_number):
        for target in config.continuation_targets_pct:
            ...
            for stop in config.stop_candidates_pct:
                ...
                elif outcome == "initial_stop" and initial_stop_index is not None:
                    if candidate_index is not None and candidate_index < initial_stop_index:
    ...
    try:
        ...
        for symbol in ALL_SYMBOLS:
            ...
            for signal in pending:
                ...
                for event, action_index in cycles:
                    if event.status == "reclaimed_peak" and action_index is not None:
                        _accumulate_action(
                            path=path,
                            action_type="peak_reclaim",
                            action_number=event.cycle_no,
                            action_index=action_index,
                            config=config,
                            tradeoff=tradeoff,
                            room_samples=rooms,
                        )
                for event, action_index in visits:
                    if event.status == "recovered_plus_0p10" and action_index is not None:
                        _accumulate_action(
                            path=path,
                            action_type="entry_recovery",
                            action_number=event.visit_no,
                            action_index=action_index,
                            config=config,
                            tradeoff=tradeoff,
                            room_samples=rooms,
