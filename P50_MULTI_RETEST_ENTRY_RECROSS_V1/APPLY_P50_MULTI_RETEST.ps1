...
Write-Host "============================================================="
Write-Host "P50 MULTI-RETEST / ENTRY RECROSS V1 - FAIL-CLOSED PRECHECK"
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "p50_multi_retest_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P50_MULTI_RETEST_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
...
Write-Host "Resulting research revision: P50 MULTI-RETEST / ENTRY RECROSS V1"
