...
class EntryBotRuntime:
    ...
    def _warmup(self) -> None:
        ...
        total = len(self.config.working_symbols)
        for index, symbol in enumerate(self.config.working_symbols, start=1):
            if self._stop.is_set():
                return
            self._set_state(
                ScannerState.WARMUP,
                f"Warm-up {index}/{total}: {symbol} · 5m/15m/60m + OI",
            )
        ...
        self._set_state(
            ...
            "Historical warm-up complete; collecting live 4+1 minute tape windows…",
        )
    ...
    def _get_json(
        ...
        with urllib.request.urlopen(request, timeout=20.0, context=_HTTPS_CONTEXT) as response:
