...
Write-Host "============================================================="
Write-Host "ENTRY BOT LIVE SCANNER P48 V1.3 FULL OVERLAY GATE - PRECHECK"
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "entry_bot_p48_overlay_" + [Guid]::NewGuid().ToString("N")
)
$PreviousMypyPath = $env:MYPYPATH
try {
    # Build the exact post-patch project layout for static checks. Ruff/isort
    # classifies first-party imports from path topology, so checking files under
    # patch\payload directly is NOT equivalent to checking src\ + tests\.
    ...
    Push-Location $Stage
    try {
        # Mirrors scripts\check_windows.ps1 exactly for Ruff.
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\ENTRY_BOT_LIVE_SCANNER_P48_V1_3_$Timestamp"
$BackedUp = 0
...
Write-Host ""
Write-Host "Then start Workbench and enable: BOT MODE - 10 ASSETS"
