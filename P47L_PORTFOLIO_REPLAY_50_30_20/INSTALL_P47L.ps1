...
function Fail([string]$Message) {
    throw "P47L install aborted: $Message"
}
...
Write-Host "P47L: verify payload hashes"
...
try {
    ...
    Write-Host "P47L: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47L: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "P47L: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "P47L: precheck green; apply real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47L_" + $stamp)
    foreach ($file in Get-ChildItem -LiteralPath $payloadRoot -Recurse -File) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47L installed successfully."
