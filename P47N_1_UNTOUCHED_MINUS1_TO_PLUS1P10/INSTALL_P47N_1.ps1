...
function Fail([string]$Message) {
    throw "P47N.1 install aborted: $Message"
}
...
$targets = @(
    "src\bybit_workbench\research\untouched_minus1_plus110_v26.py",
    "tests\test_untouched_minus1_plus110_v26.py",
    "scripts\research_untouched_minus1_plus110_all9_windows.ps1",
    "docs\P47N_UNTOUCHED_MINUS1_PLUS110_PROTOCOL_RU.md"
)
...
Write-Host "P47N.1: verify payload hashes"
$hashRows = @()
foreach ($line in Get-Content -LiteralPath $hashFile) {
    ...
    if ([System.IO.Path]::IsPathRooted($relative)) { Fail "absolute hash path forbidden: $relative" }
    if ($relative -like "payload\*") { Fail "SHA256 path must be relative to payload root: $relative" }
...
$tempRoot = Join-Path $env:TEMP ("cripta-p47n-overlay-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "P47N.1: create code-only post-patch overlay"
    ...
    foreach ($dir in $projectDirs) {
        ...
        if (Test-Path -LiteralPath $sourceDir) {
            ...
            if ($LASTEXITCODE -ge 8) { Fail "robocopy overlay failed for $dir with code $LASTEXITCODE" }
        }
    }
    ...
    Write-Host "P47N.1: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    $null = [System.Management.Automation.Language.Parser]::ParseFile(
        (Join-Path $tempRoot "scripts\research_untouched_minus1_plus110_all9_windows.ps1"),
        [ref]$tokens,
        [ref]$errors
    )
    ...
    Write-Host "P47N.1: precheck green; apply real files"
    foreach ($file in $payloadFiles) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47N.1 installed successfully."
    Write-Host "No existing project files were overwritten; this patch only adds new research files."
