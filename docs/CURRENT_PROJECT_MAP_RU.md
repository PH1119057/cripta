...
**Базовый проверенный GitHub commit перед P1:** `ae389900869ee0aadc63d5eaa4b94f7a1066460a`
**Актуальный установленный commit:** определяется каноническим
`PROJECT_GIT_HEAD.txt`; будущий SHA не записывается до создания commit.  
**Статус:** краткая каноническая карта проекта + обязательные архитектурные инварианты.

## Изменение карты 2.2: непрерывная жизнь сигнала

`signal_id` является постоянным корнем причинной истории от появления сигнала до
решения, заявки, fill, позиции, выхода и дальнейшего рыночного наблюдения.
`OBSERVED_CONTEXT` означает существовавший причинный advisory-контекст без
доказательства торгового использования; `CONSUMED_CONTEXT` допустим только когда
стратегия действительно прочитала контекст как input решения. Полный контракт:
`docs/SIGNAL_LIFECYCLE_CONTRACT_RU.md`.

Канонический порядок управления изменениями и GitHub-синхронизации:
