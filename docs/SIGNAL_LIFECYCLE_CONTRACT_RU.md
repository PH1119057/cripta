...
**Основание V1.4:** owner decision 2026-09-13 — карточка фактически открытой позиции обязана причинно сохранять движение глобальной Entry-зоны после fill как наблюдение без торгового эффекта.
...
### 12.1. Движение глобальной Entry-зоны после fill

Для каждой фактически открытой позиции карточка обязана различать две сущности:

1. **замороженный снимок глобальной Entry-геометрии**, которая реально привела к `StrategySignal` / fill;
2. **последующую причинную траекторию той же глобальной Entry-зоны** после fill.

Замороженный снимок Entry не изменяется задним числом и сохраняет точные `zone/geometry` source refs, границы, опорный экстремум, параметры геометрии и `entry_plan_fingerprint`, использованные в момент Entry.

После fill система продолжает по закрытым свечам причинно пересчитывать ту же глобальную геометрию, которая определена owning `EntryPlan`, и сохраняет append-only наблюдения за линией исходной Entry-зоны. Такое наблюдение не создаёт новый Entry, не переписывает исходную Entry-геометрию и не меняет Strategy.

Для каждого наблюдения необходимо сохранять минимум:

```text
observed_at
position_id / signal_id / strategy_id/version
entry_plan_fingerprint
entry_zone_lineage_id или эквивалентную точную причинную связь
current zone source refs / causal candle refs
границы и опорный экстремум текущего состояния глобальной зоны
смещение относительно предыдущего состояния
смещение относительно замороженной Entry-зоны
направление смещения относительно позиции: WITH / AGAINST / FLAT / UNKNOWN
признак сохранения или разрыва причинной непрерывности исходной зоны
```

Направление трактуется симметрично:

- для `LONG` устойчивое смещение глобальной зоны поддержки вверх = `WITH`, вниз = `AGAINST`;
- для `SHORT` устойчивое смещение глобальной зоны сопротивления вниз = `WITH`, вверх = `AGAINST`.

`AGAINST` означает наблюдаемый признак повышенного давления против позиции; `WITH` означает наблюдаемый положительный структурный контекст. Эти значения обязаны быть доступны карточке позиции и последующему Analyst/research.

Нельзя молча считать новой версией той же Entry-зоны любую актуальную H9-зону по symbol. Если причинная непрерывность исходной зоны нарушена, карточка фиксирует разрыв lineage и отдельно начинает наблюдать новую структуру, не подменяя ею замороженную Entry-зону.

На текущем этапе это **OBSERVE-only**. Движение глобальной Entry-зоны не имеет права само по себе открывать/закрывать позицию, двигать stop/trailing или изменять Exit. Любой торговый эффект требует отдельного research, owner decision и новой owner-approved Strategy/Exit version.
