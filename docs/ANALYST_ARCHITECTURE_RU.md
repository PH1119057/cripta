...
Для каждой фактически открытой позиции Supervisor/наблюдательный контур обязан сохранять не только замороженную глобальную Entry-геометрию, по которой произошёл вход, но и причинную временную траекторию **той же исходной глобальной Entry-зоны** после fill: её границы/опорный экстремум, смещение относительно предыдущего состояния и исходного Entry, направление `WITH / AGAINST / FLAT / UNKNOWN`, а также сохранение или разрыв lineage.

Для `LONG` постепенное смещение глобальной зоны поддержки вниз является наблюдаемым признаком давления против позиции, вверх — положительного структурного контекста. Для `SHORT` трактовка зеркальная: смещение глобальной зоны сопротивления вверх — против позиции, вниз — в её сторону.

Эта информация обязательна для карточки позиции и последующего Analyst/research, но является `OBSERVE-only`: Supervisor не становится owner Exit и не двигает stop/trailing по собственной инициативе.
