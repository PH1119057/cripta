...
Write-Host "============================================================="
Write-Host "P50 MULTI-RETEST / ENTRY RECROSS V1.1 RUFF FIX - FAIL-CLOSED PRECHECK"
...
Write-Host "V1.1 fixes patch-preparation Ruff SIM102 only; research semantics unchanged."
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "p50_multi_retest_v1_1_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P50_MULTI_RETEST_V1_1_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
...
Write-Host "Resulting research revision: P50 MULTI-RETEST / ENTRY RECROSS V1.1 (Ruff prep fix; semantics unchanged)"
