...
Write-Host "============================================================="
Write-Host "P45 MULTI-TOUCH S/R ZONES - PRECHECK"
...
if (Test-Path $python) {
    ...
    if ($pythonFiles.Count -gt 0) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "Ruff payload precheck failed; project files were not modified." }
...
$backupRoot = Join-Path $ProjectRoot "patch_backups\P45_MULTI_TOUCH_SR_$stamp"
$backedUp = $false
foreach ($item in $manifest) {
    $relative = ([string]$item.relative_path).Replace([char]'/', [IO.Path]::DirectorySeparatorChar)
    $source = Join-Path $PayloadRoot $relative
...
Write-Host "  powershell -ExecutionPolicy Bypass -File .\scripts\research_multi_touch_sr_windows.ps1"
Write-Host ""
Write-Host "Expected report:"
Write-Host "  reports\multi_touch_sr_p45\ENTRY_V1_20260518_20260816.zip"
Write-Host "============================================================="
