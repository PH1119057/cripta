...
function Fail([string]$Message) {
    throw "P47J.1 install aborted: $Message"
}
...
$targets = @(
    ...
    "tests\test_early_protection_minus01_v23.py",
    "scripts\research_early_protection_minus01_all9_windows.ps1"
)
...
Write-Host "P47J.1: verify payload hashes"
...
$tempRoot = Join-Path $env:TEMP ("cripta-p47j1-overlay-" + [guid]::NewGuid().ToString("N"))
$backupRoot = $null
...
try {
    ...
    Write-Host "P47J.1: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47J.1: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    $null = [System.Management.Automation.Language.Parser]::ParseFile(
        (Join-Path $tempRoot "scripts\research_early_protection_minus01_all9_windows.ps1"),
        [ref]$tokens,
        [ref]$errors
    )
    ...
    Write-Host "P47J.1: precheck green; apply real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47J_1_" + $stamp)
    foreach ($file in Get-ChildItem -LiteralPath $payloadRoot -Recurse -File) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47J.1 installed successfully."
