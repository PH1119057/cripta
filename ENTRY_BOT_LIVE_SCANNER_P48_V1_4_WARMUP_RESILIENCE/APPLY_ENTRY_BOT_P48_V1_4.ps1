...
Write-Host "============================================================="
Write-Host "ENTRY BOT P48 V1.4 WARMUP RESILIENCE - PRECHECK"
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "entry_bot_p48_v14_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
try {
    ...
    try {
        ...
        & $Python -m pytest -q -c (Join-Path $ProjectRoot "pyproject.toml") `
            "tests\test_entry_bot_live_scanner.py" "tests\test_gui_smoke.py"
        if ($LASTEXITCODE -ne 0) {
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\ENTRY_BOT_P48_V1_4_$Timestamp"
$BackedUp = 0
