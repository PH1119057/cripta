...
zone_events=/tmp/independent_zone_touch_outcomes.csv
report_root=/srv/cripta/reports/exact_touch_zone_structure_v1/ALL9_24H_20260825
mkdir -p "$report_root/logs"
run_symbol() {
  symbol="$1"
  cd "$source_root"
  PYTHONPATH=src "$python_bin" -m bybit_workbench.research.exact_touch_zone_structure \
    --signals "/tmp/${symbol}_exact_touch.csv" \
    --zone-events "$zone_events" \
    --raw-symbol-dir "$raw_root/$symbol" \
    --output-dir "$report_root/$symbol" \
    --symbol "$symbol" --fraction 1.0 --horizon-hours 24 \
    --activation-pct 0.10 --target-pct 1.10 --stop-pct 1.00 \
    >"$report_root/logs/${symbol}.log" 2>&1
}
export -f run_symbol
export source_root python_bin raw_root zone_events report_root
printf '%s\n' \
