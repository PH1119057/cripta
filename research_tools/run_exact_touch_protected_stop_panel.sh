...
report_root=/srv/cripta/reports/exact_touch_protected_stop_v1/ALL9_24H_20260825
mkdir -p "$report_root/logs"
run_symbol() {
  symbol="$1"
  cd "$source_root"
  PYTHONPATH=src "$python_bin" -m bybit_workbench.research.exact_touch_protected_stop \
    ...
    --initial-stop-pct 1.0 --activation-pct 0.15 \
    --protected-floor-pct 0.10 --target-pct 1.10 \
