...
def _summary_markdown(summaries: list[dict[str, Any]], contract_hash: str) -> str:
    ...
    for row in all9:
        lines.append(
            f"| {float(row['min_adverse_depth_pct']):.2f}% | "
            f"{float(row['rebound_confirmation_pct']):.2f}% | "
            f"{row['triggered']} | {row['trigger_rate_pct']} | "
            f"{row['secondary_structural_stop_rate_pct']} | "
            f"{row['reached_plus_0.50_pct']} | {row['reached_plus_1.00_pct']} | "
            f"{row['reached_plus_2.00_pct']} | {row['reached_plus_3.00_pct']} |"
        )
    lines.extend(
        [
            "",
