...
function Fail([string]$Message) {
    throw "SE1.3 install aborted: $Message"
}
...
Write-Host "SE1.3: verify payload hashes"
$hashRows = @()
...
try {
    ...
    Write-Host "SE1.3: create final-state code overlay"
    ...
    # Full pytest contains research-truth tests whose fixtures intentionally live
    # under reports\. The overlay must receive a private read-only snapshot of
    # those small machine-truth artifacts; never junction real reports into tests.
    # Heavy raw datasets/archives remain outside the overlay.
    $reportsSource = Join-Path $projectRoot "reports"
    if (Test-Path -LiteralPath $reportsSource) {
        Write-Host "SE1.3: snapshot small report fixtures into overlay"
    ...
    Write-Host "SE1.3: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "SE1.3: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "SE1.3: overlay green; apply additive files"
    foreach ($file in $payloadFiles) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "SE1.3 installed successfully."
