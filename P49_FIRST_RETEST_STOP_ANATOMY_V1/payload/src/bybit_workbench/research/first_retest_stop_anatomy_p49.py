...
def analyze_first_retest(
    ...
    *,
    ...
    if retest_stop_index is not None:
        ...
        after_moves = path.moves_pct[confirm_index:]
        max_after_confirm = max(after_moves) if after_moves else None
        min_after_confirm = min(after_moves) if after_moves else None
        post_targets = {
            target: _first_target_before_stop(
                path,
                target,
...
def build_post_retest_policy_rows(
    ...
    for activation in config.activation_levels_pct:
        activation_events = [
            event
            for event in events
            if event.activation_pct == activation and event.status == "retest_confirmed"
        ]
        for stop in config.stop_candidates_pct:
            for target in config.continuation_targets_pct:
                ...
                for event in activation_events:
                    key = (event.symbol, event.touch_at)
                    path = paths[key]
                    outcome, target_preserved = _simulate_stop_after_confirm(
                        path,
                        event,
                        stop,
                        target,
                    ...
                    baseline_runners += int(baseline_runner)
                    immediate += int(outcome == "immediate_exit")
                    tightened_stops += int(outcome in {"immediate_exit", "stop"})
                    censored += int(outcome == "data_end")
                    if baseline_runner:
                        preserved += int(target_preserved is True)
                        killed += int(target_preserved is False)
                rows.append(
                    {
                        "activation_pct": activation,
                        "stop_after_retest_confirm_pct": stop,
                        "continuation_target_pct": target,
                        "eligible_confirmed_retests": eligible,
                        "baseline_runners": baseline_runners,
                        "runner_preserved": preserved,
                        "runner_killed": killed,
                        "runner_preservation_pct": _pct(preserved, baseline_runners),
                        "runner_kill_pct": _pct(killed, baseline_runners),
                        "immediate_exit_at_confirmation": immediate,
                        "tightened_stop_exits": tightened_stops,
                        "censored": censored,
                    }
                )
    return rows
...
def run_research(root: Path, output_dir: Path, config: P49Config) -> dict[str, Any]:
    ...
    reporter.emit(processed=0, total=total_work, force=True, detail="raw-trade path anatomy")

    events: list[FirstRetestEvent] = []
    path_by_key: dict[tuple[str, str], PathSeries] = {}
    for index, signal in enumerate(signals, start=1):
        ...
        for activation in config.activation_levels_pct:
            events.append(analyze_first_retest(path, activation_pct=activation, config=config))
        cache = cache_by_symbol[signal.symbol]
        reporter.emit(
            processed=index,
            total=total_work,
            detail=(
                f"symbol={signal.symbol} cache_hits={cache.hits} "
    ...
    policy_rows = build_post_retest_policy_rows(path_by_key, events_tuple, config)
    ...
    summary: dict[str, Any] = {
        "research_version": "P49_FIRST_RETEST_STOP_ANATOMY_V1",
...
def main() -> int:
    ...
    if output_dir is None:
        ...
        output_dir = root / "reports" / "first_retest_stop_anatomy_p49" / f"ALL9_{stamp}"
