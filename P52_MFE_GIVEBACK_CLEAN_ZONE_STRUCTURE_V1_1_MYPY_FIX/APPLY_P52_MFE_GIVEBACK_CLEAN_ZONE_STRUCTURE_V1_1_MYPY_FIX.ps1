...
Write-Host "============================================================="
Write-Host "P52 MFE + GIVEBACK + CLEAN ZONE STRUCTURE V1.1 MYPY FIX - FAIL-CLOSED"
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P52_STRUCTURE_V1_1_MYPY_FIX_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
...
Write-Host "============================================================="
Write-Host "P52 V1.1 MYPY FIX PATCH APPLIED"
