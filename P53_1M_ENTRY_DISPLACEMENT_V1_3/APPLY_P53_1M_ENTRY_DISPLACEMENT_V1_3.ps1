...
$PatchName = "P53_1M_ENTRY_DISPLACEMENT_V1_3"
$ProjectRoot = (Get-Location).Path
$PatchRoot = $PSScriptRoot
$ManifestPath = Join-Path $PatchRoot "MANIFEST.json"
$PayloadRoot = Join-Path $PatchRoot "payload"
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Cripta_P53_1_3_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "Baseline: bybit-workbench 0.8.5 / P48.2 + accepted P53 V1.2"
    Write-Host "Research only. Downloads: DISABLED."
    Write-Host "Fix: continuous previous-close 1m candle semantics; exact 1m->5m OHLCV gate remains strict."
    ...
    foreach ($required in @("src", "tests", "scripts", "docs", "patches", "pyproject.toml", "uv.lock", "bybit_workbench.spec")) {
    ...
    Write-Host "[1/7] Verify accepted P53 V1.2 baseline hashes"
    ...
    Write-Host "[2/7] Verify installer and payload hashes"
    $expectedInstaller = ([string]$manifest.installer_sha256).ToLowerInvariant()
    $actualInstaller = (Get-FileHash -Algorithm SHA256 -LiteralPath $PSCommandPath).Hash.ToLowerInvariant()
    if ($actualInstaller -ne $expectedInstaller) { Fail "Installer hash mismatch expected=$expectedInstaller actual=$actualInstaller" }
    ...
    foreach ($name in @("src", "tests", "scripts", "docs", "patches", "pyproject.toml", "uv.lock", "bybit_workbench.spec")) {
    ...
    try {
        ...
        Invoke-Checked "overlay pytest except report-bound MAYAK truth" { & $Python -m pytest --ignore="tests\test_mayak_research_truth.py" }
    }
    finally {
    ...
    Write-Host "Resulting product version remains 0.8.5 / P48.2; P53 research overlay is V1.3."
