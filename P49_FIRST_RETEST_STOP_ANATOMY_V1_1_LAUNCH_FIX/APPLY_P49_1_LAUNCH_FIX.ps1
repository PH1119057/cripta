...
Write-Host "============================================================="
Write-Host "P49.1 FIRST RETEST LAUNCH FIX - FAIL-CLOSED PRECHECK"
Write-Host "Project: $ProjectRoot"
Write-Host "Baseline: bybit-workbench 0.8.5 + P49 V1 installed"
Write-Host "Fix: attach leading-negative stop CSV to argparse option"
Write-Host "Research only. Downloads: DISABLED."
Write-Host "Entry V1 / P46 / P49 research semantics / Exit / Risk / live Execution unchanged."
Write-Host "reports\ and user data are not modified by this installer."
Write-Host "============================================================="
...
if (-not $InitText.Contains('__version__ = "0.8.5"')) {
    throw "Unexpected project version. P49.1 requires bybit-workbench 0.8.5."
}
...
foreach ($Row in $BaselineRows) {
    ...
    if (-not (Test-Path $Target -PathType Leaf)) {
        throw "Required P49 V1 baseline file missing: $Relative"
    }
    $Actual = (Get-FileHash -Algorithm SHA256 $Target).Hash.ToLowerInvariant()
    $Expected = ([string]$Row.SHA256).Trim().ToLowerInvariant()
    if ($Actual -ne $Expected) {
        throw "Baseline hash mismatch: $Relative. Current C:\cripta differs from P49 V1; rebuild patch from current source."
    }
}
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "p49_1_launch_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P49_1_LAUNCH_FIX_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
...
Write-Host "Resulting research revision: P49.1 launcher fix"
Write-Host "Package version remains bybit-workbench 0.8.5."
Write-Host "No report, market-data, calibration, Entry, Exit or Risk file was changed."
Write-Host ""
