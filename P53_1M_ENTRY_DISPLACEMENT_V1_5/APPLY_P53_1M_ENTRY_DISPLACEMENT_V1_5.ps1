...
$PatchName = "P53_1M_ENTRY_DISPLACEMENT_V1_5"
$ProjectRoot = (Get-Location).Path
$PatchRoot = $PSScriptRoot
$ManifestPath = Join-Path $PatchRoot "MANIFEST.json"
$PayloadRoot = Join-Path $PatchRoot "payload"
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Cripta_P53_1_5_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "Baseline: bybit-workbench 0.8.5 / P48.2 + accepted P53 V1.4"
    Write-Host "Research only. Downloads: DISABLED."
    Write-Host "Fix: seed exact first public-trade archive boundary, not earlier 5m warm-up."
    ...
    Write-Host "[1/7] Verify accepted P53 V1.4 baseline hashes"
    ...
    Write-Host "Resulting product version remains 0.8.5 / P48.2; P53 research overlay is V1.5."
