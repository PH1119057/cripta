...
def test_run_p53_passes_frozen_five_minute_series_for_archive_boundary_seed() -> None:
    ...
    keyword = next(
        item for item in load_calls[0].keywords if item.arg == "frozen_five_minute"
    )
    assert isinstance(keyword.value, ast.Name)
    assert keyword.value.id == "five"



def test_boundary_seed_uses_matching_archive_day_not_first_warmup_candle() -> None:
    warmup_at = datetime(2026, 5, 10, tzinfo=UTC)
    boundary_at = datetime(2026, 5, 17, tzinfo=UTC)
    five = (
        _candle(
            timeframe="5",
            opened_at=warmup_at,
    ...
    opened_at, seed = p53._boundary_seed_from_frozen_five(five, date(2026, 5, 17))
    assert opened_at == boundary_at
    assert seed == Decimal("3.483")


def test_boundary_seed_fails_closed_when_matching_five_minute_candle_is_missing() -> None:
    five = (
        _candle(
            timeframe="5",
    ...
    try:
        p53._boundary_seed_from_frozen_five(five, date(2026, 5, 17))
    except ValueError as exc:
        assert "archive boundary" in str(exc)
        assert "matches=0" in str(exc)
    else:
        raise AssertionError("missing frozen 5m archive-boundary candle must fail closed")
