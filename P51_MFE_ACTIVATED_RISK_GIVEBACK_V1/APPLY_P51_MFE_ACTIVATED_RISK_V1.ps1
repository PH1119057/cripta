...
Write-Host "============================================================="
Write-Host "P51 MFE-ACTIVATED RISK / GIVEBACK V1 - FAIL-CLOSED PRECHECK"
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "p51_mfe_risk_v1_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P51_MFE_RISK_V1_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
