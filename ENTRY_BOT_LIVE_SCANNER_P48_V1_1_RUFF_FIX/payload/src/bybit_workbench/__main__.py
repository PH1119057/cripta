...
def main(argv: list[str] | None = None) -> int:
    ...
    entry_bot_runtime: EntryBotRuntime | None = None
    ...
    if settings.mode is AppMode.REPLAY:
        ...
        if settings.testnet_execution_allowed:
            ...
            entry_bot_runtime = EntryBotRuntime(settings)
    ...
    def set_mainnet_endpoint(endpoint: str) -> str:
        ...
        if entry_bot_runtime is not None:
            entry_bot_runtime.reconfigure(updated)
    ...
    def pump_runtimes() -> None:
        ...
        if entry_bot_runtime is not None:
            for signal in entry_bot_runtime.drain_signals():
                model.append_strategy_decision(
                    f"{signal.touch_at.isoformat()} · ENTRY V1 CORE · "
                    f"{signal.symbol} {signal.direction} @ {signal.entry_price}"
                )
    ...
    window = create_main_window(
        ...
        start_entry_bot=None if entry_bot_runtime is None else entry_bot_runtime.start,
        stop_entry_bot=(
            None if entry_bot_runtime is None else entry_bot_runtime.request_stop
        ),
        get_entry_bot_snapshot=(
            None if entry_bot_runtime is None else entry_bot_runtime.snapshot
        ),
