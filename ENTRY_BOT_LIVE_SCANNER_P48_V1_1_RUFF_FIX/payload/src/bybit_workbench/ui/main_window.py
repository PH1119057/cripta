    ...
    *,
    ...
    class MainWindow(QMainWindow):
        ...
        def _build_right_panel(self) -> QWidget:
            ...
            note = QLabel(
                "Двойной щелчок по монете открывает её в основном графике. "
                "Сигнал здесь пока только фиксируется: автоматическая Mainnet-заявка "
                "закрыта до отдельного production-equivalence gate."
            )
            note.setWordWrap(True)
        ...
        def _set_bot_mode(self, enabled: bool) -> None:
            ...
            if enabled and start_entry_bot is not None:
                self._start_entry_bot()
        ...
        def _refresh_connection_controls(self, state: Any) -> None:
            ...
            elif self._connection_action in {"connecting", "reconnecting"}:
                # The runtime moves through SYNCING/DEGRADED while it owns reconnects.
                # A bare DISCONNECTED state means that attempt has ended.
                self._connection_action = "idle"
            self._set_connection_activity(
