...
Write-Host "============================================================="
Write-Host "P51 MFE-ACTIVATED RISK / GIVEBACK V1.1 MYPY FIX - FAIL-CLOSED PRECHECK"
Write-Host "Project: $ProjectRoot"
Write-Host "Baseline: bybit-workbench 0.8.5 + accepted P50 V1.2 source"
Write-Host "Research only. Downloads: DISABLED."
Write-Host "Adds fixed MFE/giveback anatomy; no optimizer and no OOS access."
Write-Host "V1.1 fixes patch-preparation mypy typing only; research semantics unchanged."
Write-Host "MFE: +0.25,+0.50,+0.75,+1.00 percent"
Write-Host "Candidate stops: -0.75,-0.60,-0.50 percent"
Write-Host "Targets: +1.10,+2.00,+3.00 percent"
Write-Host "Primary action: first causal +0.10 Entry recovery after MFE."
Write-Host "Entry V1 / frozen P46 / live Execution / Exit / Risk unchanged."
Write-Host "Reserved BNB/AVAX/SUI/AAVE/LTC OOS assets are NOT touched."
...
if (-not $InitText.Contains('__version__ = "0.8.5"')) {
    throw "Unexpected project version. P51 requires bybit-workbench 0.8.5."
}
...
foreach ($Row in $BaselineRows) {
    ...
    if (-not (Test-Path $Target -PathType Leaf)) {
        throw "Required accepted P50 baseline file missing: $Relative"
    }
    $Actual = (Get-FileHash -Algorithm SHA256 $Target).Hash.ToLowerInvariant()
    $Expected = ([string]$Row.SHA256).Trim().ToLowerInvariant()
    if ($Actual -ne $Expected) {
        throw "Baseline hash mismatch: $Relative. Rebuild P51 from current C:\cripta source."
    }
}
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "p51_mfe_risk_v1_1_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
try {
    ...
    $StageLauncher = Join-Path $Stage "scripts\research_mfe_activated_risk_p51_windows.ps1"
    $Tokens = $null
    $Errors = $null
    [System.Management.Automation.Language.Parser]::ParseFile(
        $StageLauncher,
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P51_MFE_RISK_V1_1_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
...
Write-Host "Resulting research revision: P51 MFE-ACTIVATED RISK / GIVEBACK V1"
...
Write-Host ""
Write-Host "Then run P51:"
Write-Host "  powershell -ExecutionPolicy Bypass -File .\scripts\research_mfe_activated_risk_p51_windows.ps1"
Write-Host ""
Write-Host "P51 resumable output:"
Write-Host "  reports\mfe_activated_risk_p51\ALL9_P51_WORKING"
Write-Host "============================================================="
