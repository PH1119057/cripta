...
def _load_exact_baseline(
    ...
    counts: defaultdict[BaselineOutcome, int] = defaultdict(int)
...
def _build_economic_rows(
    ...
    for signal_row in first_rule_rows:
        for scope in _scope_labels(signal_row.symbol, signal_row.direction):
            key = scope, signal_row.mfe_milestone_pct, signal_row.stop_pct
            ...
            bucket["signals_with_rule_action"] += float(signal_row.action_visit_no is not None)
            bucket["actions_before_baseline_outcome"] += float(
                signal_row.acted_before_baseline_outcome
            )
            bucket["baseline_winners_acted"] += float(
                signal_row.acted_before_baseline_outcome
                and signal_row.baseline_outcome == "reached_plus_1p10"
            )
            bucket["baseline_losers_acted"] += float(
                signal_row.acted_before_baseline_outcome
                and signal_row.baseline_outcome == "hit_minus_1p00"
            )
            bucket["saved_baseline_losers"] += float(signal_row.saved_baseline_loser)
            bucket["killed_baseline_winners"] += float(signal_row.killed_baseline_winner)
            bucket["candidate_stop_before_baseline_outcome"] += float(
                signal_row.candidate_stop_before_baseline_outcome
            )
            bucket["illustrative_delta_usd"] += signal_row.illustrative_delta_usd
    ...
    for (scope, milestone, stop), bucket in sorted(aggregate.items()):
        ...
        if scope == "ALL9":
            aggregate_row["illustrative_all9_baseline_net_usd"] = economics[
                "illustrative_aggregate_net_usd"
            ]
            aggregate_row["illustrative_all9_net_after_rule_usd"] = round(
                economics["illustrative_aggregate_net_usd"] + delta, 6
            )
        rows.append(aggregate_row)
    return [output_row for output_row in rows if output_row["scope"] == "ALL9"], rows
