...
function Fail([string]$Message) {
    throw "P47L.1 install aborted: $Message"
}
...
$requiredBaseline = @(
    ...
    "scripts\research_early_failure_puncture_all9_windows.ps1",
...
$targets = @(
    ...
    "tests\test_portfolio_replay_v25.py",
    "scripts\research_portfolio_replay_all9_windows.ps1",
    "docs\P47L_PORTFOLIO_REPLAY_PROTOCOL_RU.md"
)
...
Write-Host "P47L.1: verify payload hashes"
...
$tempRoot = Join-Path $env:TEMP ("cripta-p47l-overlay-" + [guid]::NewGuid().ToString("N"))
$backupRoot = $null
...
try {
    ...
    Write-Host "P47L.1: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47L.1: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    $null = [System.Management.Automation.Language.Parser]::ParseFile(
        (Join-Path $tempRoot "scripts\research_portfolio_replay_all9_windows.ps1"),
        [ref]$tokens,
        [ref]$errors
    )
    ...
    Write-Host "P47L.1: precheck green; apply real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47L_1_" + $stamp)
    foreach ($file in Get-ChildItem -LiteralPath $payloadRoot -Recurse -File) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47L.1 installed successfully."
