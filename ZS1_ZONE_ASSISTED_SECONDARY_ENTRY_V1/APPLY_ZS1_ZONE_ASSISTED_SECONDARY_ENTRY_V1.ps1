...
try {
    Write-Host "============================================================="
    Write-Host "ZS1 ZONE-ASSISTED SECONDARY ENTRY V1 - FAIL-CLOSED INSTALLER"
    ...
    Write-Host "ZS1: verify frozen report prerequisites"
    $ExpectedSources = @{
    ...
    Write-Host "ZS1: verify payload hashes"
    ...
    Write-Host "ZS1: create final-state overlay"
    $Overlay = Join-Path $env:TEMP ("zs1_overlay_" + [Guid]::NewGuid().ToString("N"))
    ...
    Write-Host "ZS1: PowerShell 5.1 syntax precheck"
    ...
    if ($LASTEXITCODE -ne 0) { Fail "py_compile overlay precheck failed" }

    Write-Host "ZS1: Ruff full overlay"
    Push-Location $Overlay
    try {
        & $Ruff check src tests scripts
        if ($LASTEXITCODE -ne 0) { Fail "Ruff overlay precheck failed" }

        Write-Host "ZS1: mypy full overlay source"
        & $Mypy src
        if ($LASTEXITCODE -ne 0) { Fail "mypy overlay precheck failed" }

        Write-Host "ZS1: targeted pytest"
        $env:PYTHONPATH = (Join-Path $Overlay "src")
        & $Python -m pytest -q tests\test_secondary_entry_zone_scale_zs1.py
        if ($LASTEXITCODE -ne 0) { Fail "targeted pytest overlay precheck failed" }
    }
    finally {
        Pop-Location
    ...
    Write-Host "ZS1: overlay precheck PASSED; project may now be changed"

    $Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $Root ("patch_backups\ZS1_ZONE_ASSISTED_SECONDARY_ENTRY_V1_" + $Stamp)
    ...
    Write-Host "ZS1: install complete"
    Write-Host "Backup: $BackupRoot"
    Write-Host "Application version remains 0.8.5"
    Write-Host "Research version added: ZS1_ZONE_ASSISTED_SECONDARY_ENTRY_V1"
    Write-Host ""
    ...
    Write-Error $_
    Write-Host "ZS1 INSTALL FAILED. If failure occurred before 'overlay precheck PASSED', project files and reports were not touched."
    exit 1
}
finally {
    $env:PYTHONPATH = $OldPythonPath
