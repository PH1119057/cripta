...
def _direct_p52_quality(path: Path, window: str) -> list[dict[str, Any]]:
    ...
    for state in sorted(grouped):
        rows = grouped[state]
        resolved_baseline = [r for r in rows if r["baseline_outcome"] in {"reached_plus_1p10", "hit_minus_1p00"}]
        ...
        out.append(
            {
                ...
                "plus2_before_minus1_pct": _pct(sum(_truth(r["plus2_before_minus1"]) for r in rows), len(rows)),
                "plus3_before_minus1_pct": _pct(sum(_truth(r["plus3_before_minus1"]) for r in rows), len(rows)),
...
def _positive_scope_fraction(rows: list[dict[str, Any]], key_name: str, min_resolved: int) -> tuple[int, int, float | None]:
    grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in rows:
        grouped[str(row[key_name])].append(row)
    evaluable = 0
...
def run(root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    for se1_row in _iter_base_se1(sources.se1_events):
        ...
        row = {
            ...
            "structural_stop_distance_from_scale_pct": float(se1_row["structural_stop_distance_from_scale_pct"]),
    ...
    resolved_after_scale = [row for row in event_rows if row["zone_relation"] == "resolved_after_scale"]
    ...
    direct_quality.extend(_direct_p52_quality(sources.p52_first_full, "first_resolved_before_baseline_outcome"))
    ...
    _write_csv(output_dir / "p52_direct_structure_quality.csv", direct_quality, list(direct_quality[0].keys()))
    ...
    readme = (
        ...
        "production confirmation. It does not access NEW5 and does not modify Entry/Exit/Risk/live.\n"
