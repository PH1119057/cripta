...
from bybit_workbench.research.exit_state_machine_eo4 import (
    Result,
    Trade,
    ZoneEvent,
...
def _series(closes: list[float]):
    ts = (
        np.arange(len(closes), dtype=np.float64) * 60
        + datetime(2026, 5, 18, tzinfo=UTC).timestamp()
    )
...
def test_runner_is_not_closed_at_baseline_target() -> None:
    ...
    trade = Trade("X", "Long", start, start, 100.0)
    event = ZoneEvent(
        datetime.fromtimestamp(start.timestamp() + 30, tz=UTC),
        "resistance|clean_break",
    )
