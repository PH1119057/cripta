...
def test_timing_summary_handles_optional_percentiles() -> None:
    rows = [
        {"seconds_zone_to_se1_scale": 600.0},
        {"seconds_zone_to_se1_scale": 1200.0},
        {"seconds_zone_to_se1_scale": 1800.0},
    ]
    result = _timing_summary("x", rows)
    assert result["n"] == 3
    assert result["p25_minutes"] == 15.0
    assert result["median_minutes"] == 20.0
    assert result["p75_minutes"] == 25.0


def test_timing_summary_empty_rows_returns_none_percentiles() -> None:
    result = _timing_summary("x", [])
    assert result["n"] == 0
    assert result["p25_minutes"] is None
    assert result["median_minutes"] is None
    assert result["p75_minutes"] is None
