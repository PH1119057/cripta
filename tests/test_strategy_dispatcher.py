...
def snap(features: dict[str, FeatureValue], quality: DispatcherDataQuality = DispatcherDataQuality.HIGH) -> DispatcherMarketSnapshot:
...
def test_required_missing_fails_closed_to_insufficient_data() -> None:
    item = profile(ProfileRule("data.quality", RequirementMode.REQUIRED, MatchOperator.ONE_OF, ("HIGH",)))
...
def test_rejected_condition_overrides_good_soft_score() -> None:
    item = profile(
        ProfileRule("data.quality", RequirementMode.REQUIRED, MatchOperator.ONE_OF, ("HIGH",)),
        ProfileRule("market.volatility", RequirementMode.PREFERRED, MatchOperator.ONE_OF, ("NORMAL",)),
        ProfileRule("liquidation.phase", RequirementMode.REJECTED, MatchOperator.ONE_OF, ("CASCADE",)),
    )
...
def test_preferred_rules_produce_explainable_score() -> None:
    item = profile(
        ProfileRule("data.quality", RequirementMode.REQUIRED, MatchOperator.ONE_OF, ("HIGH",)),
        ProfileRule("market.volatility", RequirementMode.PREFERRED, MatchOperator.ONE_OF, ("NORMAL",), weight=2),
        ProfileRule("market.synchronization", RequirementMode.PREFERRED, MatchOperator.ONE_OF, ("LOW",)),
    )
    result = StrategyDispatcher().evaluate(
        snap({
            "data.quality": fv("HIGH"),
...
def test_low_quality_reduces_confidence_without_rewriting_market_values() -> None:
    item = profile(ProfileRule("market.volatility", RequirementMode.PREFERRED, MatchOperator.ONE_OF, ("NORMAL",)))
    result = StrategyDispatcher().evaluate(
...
def test_profile_cannot_reference_unknown_feature() -> None:
    item = profile(ProfileRule("strategy.profit", RequirementMode.PREFERRED, MatchOperator.ONE_OF, ("GOOD",)))
...
def test_profile_rejects_typo_in_categorical_value() -> None:
    item = profile(
        ProfileRule("market.volatility", RequirementMode.PREFERRED, MatchOperator.ONE_OF, ("NROMAL",))
    )
