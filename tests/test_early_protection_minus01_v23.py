...
def test_activation_then_data_end_preserves_activation_timestamp() -> None:
    result = scan(
        _signal(),
        {"2026-06-01": Path("day")},
        FakeCache(_tape([0.0, 0.11, 0.08, 0.20])),
        Config(),
    )
    assert result.outcome == "data_end_after_activation"
    assert result.activation_at is not None
