...
def test_dashboard_live_actions_use_strategy_owned_symbols_not_monitor_checkbox_gate() -> None:
...
def test_live_polling_adapts_to_visible_trade_page_and_skips_hidden_dom() -> None:
    ...
    assert (
        "open:1000,paper_open:2000,paper_closed:5000,monitor:2000,closed:10000,signals:5000" in html
    )
...
def test_live_state_requests_only_data_for_visible_trade_view() -> None:
    ...
    assert 'strategy_monitor_state() if view == "monitor" else None' in app
    assert '"entry_shadow": None' in app
