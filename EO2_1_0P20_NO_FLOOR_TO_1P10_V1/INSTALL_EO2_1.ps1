...
function Fail([string]$Message) {
    throw "EO2.1 install aborted: $Message"
}
...
Write-Host "EO2.1: verify exact current baseline"
...
Write-Host "EO2.1: verify payload hashes"
$hashRows = @()
...
try {
    ...
    Write-Host "EO2.1: create final-state code overlay"
    ...
    if (Test-Path -LiteralPath $reportsSource) {
        Write-Host "EO2.1: snapshot small report fixtures into overlay"
    ...
    Write-Host "EO2.1: PowerShell 5.1 syntax precheck"
    foreach ($script in @(
    ...
    try {
        ...
        Write-Host "EO2.1: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "EO2.1: overlay green; record new-file rollback manifest"
    $backupRoot = Join-Path $patchRoot ("backup_before_apply_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
    New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null
    @(
        "EO2.1 is a new-file-only research patch revision; research contract remains EO2 V1.",
    ...
    Write-Host "EO2.1: apply payload"
    try {
        foreach ($file in $payloadFiles) {
            $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
        ...
        Write-Host "EO2.1: apply failed; removing new EO2 files"
        foreach ($relative in $newFiles) {
    ...
    Write-Host "EO2.1: authoritative final Windows gate"
    & powershell -ExecutionPolicy Bypass -File (Join-Path $projectRoot "scripts\check_windows.ps1")
    if ($LASTEXITCODE -ne 0) {
        Write-Host "EO2.1: authoritative gate red; removing new EO2 files"
        foreach ($relative in $newFiles) {
            Remove-Item -LiteralPath (Join-Path $projectRoot $relative) -Force -ErrorAction SilentlyContinue
        }
        $applied = $false
    ...
    Write-Host "EO2.1 installed successfully."
