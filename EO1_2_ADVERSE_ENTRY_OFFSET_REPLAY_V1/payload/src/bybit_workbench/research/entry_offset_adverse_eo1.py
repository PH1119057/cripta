...
ENGINE_REVISION = "EO1_2_MEMORY_BOUNDED_STREAMING"
...
class _ScenarioState:
    ...
    first_cross_signal_move_pct: float | None = None
    first_cross_price: float | None = None
    trigger_slippage_bps_proxy: float | None = None
    seconds_touch_to_fill: float | None = None
    original_target_before_fill_at: datetime | None = None
    offset_touch_anytime_at: datetime | None = None
    protection_activated: bool = False
    protection_activation_at: datetime | None = None
    activation_tick_seen: bool = False
    exit_reason: ExitReason | None = None
    exit_at: datetime | None = None
    exit_signal_move_pct: float | None = None
    mfe_from_fill_pct: float | None = None
    mae_from_fill_pct: float | None = None
...
def _relative_move_from_signal_move(
    direction: Direction,
    signal_move: float,
    adverse_offset_pct: float,
) -> float:
    ...
    if denominator <= 0:
        raise ValueError("invalid shifted-entry denominator")
    return (signal_move - fill_signal_move) / denominator
...
def _dates_for_stream(start: datetime, hours: int) -> tuple[str, ...]:
...
def _stream_coverage(
    ...
    required_days = _dates_for_stream(signal.touch_at, config.max_path_hours)
    ...
    requested_end = signal.touch_at + timedelta(hours=config.max_path_hours)
    if not continuous_days:
        coverage_until = signal.touch_at
    elif len(continuous_days) == len(required_days):
        coverage_until = requested_end
    else:
        coverage_until = min(
            requested_end,
            _day_start(required_days[len(continuous_days)]),
        )
    return tuple(continuous_days), missing_days, coverage_until


def _iter_signal_observations_bounded(
    signal: CoreSignal,
    archive_by_day: dict[str, Path],
    continuous_days: Sequence[str],
    *,
    config: Config,
    cache: TradeDayCache,
) -> Iterator[tuple[float, float]]:
    start_ts = signal.touch_at.timestamp()
    end_ts = (signal.touch_at + timedelta(hours=config.max_path_hours)).timestamp()
...
def _record_fill(
    ...
    *,
    ...
    state.fill_at = datetime.fromtimestamp(timestamp, UTC)
    state.first_cross_signal_move_pct = signal_move
    first_cross_price = _price_from_signal_move(
        signal.direction,
        signal.entry_price,
        signal_move,
    )
    state.first_cross_price = first_cross_price
    state.trigger_slippage_bps_proxy = (
        directional_move_pct(
            signal.direction,
            state.pending_price,
            first_cross_price,
    ...
    state.seconds_touch_to_fill = max(0.0, timestamp - signal.touch_at.timestamp())
...
def _update_trade_state(
    ...
    *,
    ...
    if state.fill_at is None:
        return
    if timestamp > (state.fill_at + timedelta(hours=config.trade_horizon_hours)).timestamp():
        return

    fill_move = _relative_move_from_signal_move(
        signal.direction,
        signal_move,
        state.offset_pct,
    )
    state.mfe_from_fill_pct = (
        fill_move
        if state.mfe_from_fill_pct is None
        else max(state.mfe_from_fill_pct, fill_move)
    )
    state.mae_from_fill_pct = (
        fill_move
        if state.mae_from_fill_pct is None
        else min(state.mae_from_fill_pct, fill_move)
    )
    ...
    if not state.protection_activated:
        # Match _simulate_trade ordering exactly. A single tick that first reaches
        # both +0.10 activation and +1.10 target exits at target without claiming
        # protection activation.
        if fill_move >= config.target_pct:
            state.exit_reason = "target"
            state.exit_at = event_at
            state.exit_signal_move_pct = signal_move
            return
        if fill_move <= -config.initial_stop_pct:
            state.exit_reason = "initial_stop"
            state.exit_at = event_at
            state.exit_signal_move_pct = signal_move
            return
        if fill_move >= config.activation_pct:
            state.protection_activated = True
            state.protection_activation_at = event_at
            state.activation_tick_seen = True
        else:
            return

    if state.activation_tick_seen:
        # Existing vector replay starts floor/target checks at activation+1.
        state.activation_tick_seen = False
        return

    if fill_move <= config.positive_floor_pct:
        state.exit_reason = "positive_floor"
        state.exit_at = event_at
        state.exit_signal_move_pct = signal_move
    elif fill_move >= config.target_pct:
        state.exit_reason = "target"
        state.exit_at = event_at
        state.exit_signal_move_pct = signal_move
...
def _finalize_stream_state(
    ...
    *,
    ...
    pending_complete = pending_observed and coverage_until >= (
        signal.touch_at + timedelta(hours=config.pending_horizon_hours)
    )
    if state.fill_status is None:
        state.fill_status = (
            "pending_horizon_no_fill" if pending_complete else "data_end_no_fill"
        )

    if state.fill_status != "filled":
        return EventResult(
            symbol=signal.symbol,
            direction=signal.direction,
            touch_at=signal.touch_at,
            original_entry_price=signal.entry_price,
            adverse_offset_pct=state.offset_pct,
            scenario=_scenario_name(state.offset_pct),
            pending_entry_price=state.pending_price,
            ...
            trigger_slippage_bps_proxy=None,
            seconds_touch_to_fill=None,
            original_target_before_fill_at=state.original_target_before_fill_at,
            offset_touched_anytime_pending_72h=state.offset_touch_anytime_at is not None,
            offset_touch_anytime_at=state.offset_touch_anytime_at,
            protection_activated=False,
            protection_activation_at=None,
            exit_reason=None,
            exit_at=None,
            ...
            pending_window_complete=pending_complete,
            trade_window_complete=None,
            missing_archive_days=";".join(missing_days),
        )

    if state.fill_at is None or state.first_cross_signal_move_pct is None:
        raise ValueError("filled streaming state is missing fill metadata")
    if state.mfe_from_fill_pct is None or state.mae_from_fill_pct is None:
        raise ValueError("filled streaming state has no trade observations")

    trade_complete = state.exit_reason is not None or coverage_until >= (
        state.fill_at + timedelta(hours=config.trade_horizon_hours)
    )
    exit_reason: ExitReason = (
        state.exit_reason
        if state.exit_reason is not None
        else ("horizon" if trade_complete else "data_end")
    )
    theoretical_level: float | None = None
    if exit_reason == "target":
        theoretical_level = config.target_pct
    ...
    net_pct = (
        ...
        else gross_pct - config.illustrative_round_trip_cost_pct
    )
    pnl_usd = (
        None
        if net_pct is None
        else config.margin_usd * config.leverage * net_pct / 100.0
    )
    exit_move_observed: float | None = None
    if state.exit_signal_move_pct is not None:
        exit_price = _price_from_signal_move(
            signal.direction,
            signal.entry_price,
            state.exit_signal_move_pct,
        )
        exit_move_observed = directional_move_pct(
            signal.direction,
            state.pending_price,
    ...
    return EventResult(
        ...
        pending_entry_price=state.pending_price,
        fill_status="filled",
        fill_at=state.fill_at,
        fill_price_ideal=state.pending_price,
        first_cross_signal_move_pct=state.first_cross_signal_move_pct,
        first_cross_price=state.first_cross_price,
        trigger_slippage_bps_proxy=state.trigger_slippage_bps_proxy,
        seconds_touch_to_fill=state.seconds_touch_to_fill,
        original_target_before_fill_at=None,
        offset_touched_anytime_pending_72h=True,
        offset_touch_anytime_at=state.offset_touch_anytime_at,
        protection_activated=state.protection_activated,
        protection_activation_at=state.protection_activation_at,
        exit_reason=exit_reason,
        ...
        net_pnl_pct_after_cost_reserve=net_pct,
        pnl_usd_100_margin_10x=pnl_usd,
        mfe_from_fill_pct=state.mfe_from_fill_pct,
        mae_from_fill_pct=state.mae_from_fill_pct,
        pending_window_complete=pending_complete,
        trade_window_complete=trade_complete,
        missing_archive_days=";".join(missing_days),
    )
...
def analyze_signal_streaming(
    signal: CoreSignal,
    archive_by_day: dict[str, Path],
    config: Config,
    *,
    cache: TradeDayCache,
) -> tuple[list[EventResult], tuple[str, ...]]:
    continuous_days, missing_days, coverage_until = _stream_coverage(
        signal,
        archive_by_day,
        config,
    )
    ...
    for timestamp, signal_move in _iter_signal_observations_bounded(
        signal,
        archive_by_day,
        continuous_days,
        config=config,
        ...
        if timestamp <= pending_end_ts:
            pending_observed = True
            for state in states:
                is_baseline = abs(state.offset_pct) < 1e-12
                is_touch = signal_move <= -state.offset_pct
                if state.offset_touch_anytime_at is None and (is_baseline or is_touch):
                    state.offset_touch_anytime_at = datetime.fromtimestamp(timestamp, UTC)
                ...
                if state.fill_status is None:
                    ...
                    elif signal_move >= config.target_pct:
                        state.fill_status = "original_target_before_fill"
                        state.original_target_before_fill_at = datetime.fromtimestamp(
                            timestamp,
                            UTC,
                        )
                    elif is_touch:
    ...
    if not saw_observation:
        raise ValueError(f"no observations for {signal.symbol} {signal.touch_at}")
...
def run(project_root: Path, output_dir: Path, config: Config) -> None:
    ...
    resumed_event_rows = len(existing_rows)
    ...
    with partial_path.open("a", encoding="utf-8") as partial:
        for signal in signals:
            ...
            if key in completed:
                continue
            signal_rows, missing_days = analyze_signal_streaming(
                signal,
            ...
            internal_missing = [day for day in missing_days if day <= max_day]
            if internal_missing:
    ...
    summary_payload: dict[str, Any] = {
        ...
        "resumed_event_rows": resumed_event_rows,
