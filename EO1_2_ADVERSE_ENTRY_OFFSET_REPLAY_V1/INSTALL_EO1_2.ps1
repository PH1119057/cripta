...
function Fail([string]$Message) {
    throw "EO1.2 install aborted: $Message"
}
...
$baselineHashes = @{
    "src\bybit_workbench\research\entry_offset_adverse_eo1.py" = "fa6b2845be479110d04a477b6fe92d9c7339ad1203a5da90e7e566fdf83e9c11"
    "tests\test_entry_offset_adverse_eo1.py" = "9329ebdf20bf5581c16217978520a744a005a91939f82dda09a9f25fd608ce08"
    "scripts\research_entry_offset_adverse_eo1_all9_windows.ps1" = "cda9a7160059e55be40eeef32f0466992394b91bf5319bc3d5ee03101161da23"
    "docs\ENTRY_OFFSET_ADVERSE_EO1_PROTOCOL_RU.md" = "6ddfcb6e16b48cd9de4df02ef948d42d4cc4b3813f17e5c717c096964cb90e22"
...
Write-Host "EO1.2: verify exact current baseline"
...
Write-Host "EO1.2: verify payload hashes"
$hashRows = @()
...
$tempRoot = Join-Path $env:TEMP ("cripta-eo12-overlay-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "EO1.2: create final-state code overlay"
    ...
    if (Test-Path -LiteralPath $reportsSource) {
        Write-Host "EO1.2: snapshot small report fixtures into overlay"
    ...
    Write-Host "EO1.2: PowerShell 5.1 syntax precheck"
    foreach ($script in @(
    ...
    Write-Host "EO1.2: overlay green; create backup of overwritten EO1.1 files"
    ...
    Write-Host "EO1.2: apply payload"
    try {
        foreach ($file in $payloadFiles) {
            $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
        ...
        Write-Host "EO1.2: apply failed; restoring EO1.1 backup"
        foreach ($file in $payloadFiles) {
            $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
            $backup = Join-Path $backupRoot $relative
            $target = Join-Path $projectRoot $relative
    ...
    Write-Host "EO1.2: authoritative final Windows gate"
    & powershell -ExecutionPolicy Bypass -File (Join-Path $projectRoot "scripts\check_windows.ps1")
    if ($LASTEXITCODE -ne 0) {
        Write-Host "EO1.2: authoritative gate red; restoring EO1.1 backup"
        foreach ($file in $payloadFiles) {
            $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
            $backup = Join-Path $backupRoot $relative
            $target = Join-Path $projectRoot $relative
        ...
        $applied = $false
        Fail "scripts\check_windows.ps1 failed; EO1.1 restored"
    }

    Write-Host "EO1.2 installed successfully."
    Write-Host ("Backup: " + $backupRoot)
    Write-Host "Research logic remains EO1 V1; only the replay engine is memory-bounded."
    Write-Host "Entry, P50/P51/P53, SE1/SE2, Exit, Risk, Execution, MAYAK and live are unchanged."
    Write-Host "To resume the failed run:"
    Write-Host "powershell -ExecutionPolicy Bypass -File .\scripts\research_entry_offset_adverse_eo1_all9_windows.ps1 -ResumeDir .\reports\entry_offset_adverse_eo1\ALL9_EO1_20260822_154907"
}
finally {
