...
Write-Host "============================================================="
Write-Host "P49.2 MEMORY-BOUNDED + RESUME - FAIL-CLOSED PRECHECK"
Write-Host "Project: $ProjectRoot"
Write-Host "Baseline: bybit-workbench 0.8.5 + P49 V1 + P49.1"
Write-Host "Fix: bound raw-path memory and add resumable checkpoints"
Write-Host "Research only. Downloads: DISABLED."
Write-Host "Entry V1 / P46 / Exit / Risk / live Execution unchanged."
...
if (-not $InitText.Contains('__version__ = "0.8.5"')) {
    throw "Unexpected project version. P49.2 requires bybit-workbench 0.8.5."
}
...
foreach ($Row in $BaselineRows) {
    ...
    if (-not (Test-Path $Target -PathType Leaf)) {
        throw "Required P49.1 baseline file missing: $Relative"
    }
    $Actual = (Get-FileHash -Algorithm SHA256 $Target).Hash.ToLowerInvariant()
    $Expected = ([string]$Row.SHA256).Trim().ToLowerInvariant()
    if ($Actual -ne $Expected) {
        throw "Baseline hash mismatch: $Relative. Current C:\cripta differs from P49.1; rebuild patch from current source."
    }
}
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "p49_2_memory_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
try {
    ...
    $StageLauncher = Join-Path $Stage "scripts\research_first_retest_stop_anatomy_p49_windows.ps1"
    $Tokens = $null
    $Errors = $null
    [System.Management.Automation.Language.Parser]::ParseFile(
        $StageLauncher,
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P49_2_MEMORY_BOUNDED_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
...
Write-Host "Resulting research revision: P49.2 MEMORY BOUNDED + RESUME"
...
Write-Host ""
Write-Host "Then rerun P49:"
Write-Host "  powershell -ExecutionPolicy Bypass -File .\scripts\research_first_retest_stop_anatomy_p49_windows.ps1"
Write-Host ""
Write-Host "Default resumable work directory:"
Write-Host "  reports\first_retest_stop_anatomy_p49\ALL9_P49_WORKING"
Write-Host "Checkpoint is saved every 25 signals and at every symbol boundary."
Write-Host "============================================================="
