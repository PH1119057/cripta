...
Write-Host "============================================================="
Write-Host "P49.3.1 FULL FIRST RETEST BASIN + 3H CLEAN - FAIL-CLOSED PRECHECK"
Write-Host "Project: $ProjectRoot"
Write-Host "Baseline: bybit-workbench 0.8.5 + P49 V1 + P49.1 + P49.2"
Write-Host "Research only. Downloads: DISABLED."
Write-Host "Rebuilt clean P49.3: full retest basin, 3h path table, saved-losers vs lost-runners."
...
$Stage = Join-Path ([System.IO.Path]::GetTempPath()) (
    "p49_3_1_full_retest_overlay_" + [Guid]::NewGuid().ToString("N")
)
...
$BackupRoot = Join-Path $ProjectRoot "patch_backups\P49_3_1_FULL_RETEST_$Timestamp"
$BackedUp = 0
foreach ($Row in $Rows) {
    $Relative = ([string]$Row.Path).Replace("/", "\")
    $Target = Join-Path $ProjectRoot $Relative
...
Write-Host "Resulting research revision: P49.3 FULL FIRST RETEST BASIN + 3H V1 (P49.3.1 clean build)"
