...
## P48 V1.7 — causal tape warm-up + visible progress

Live Entry must not become ready merely because five wall-clock minutes elapsed after REST
warm-up. The frozen flow gate consumes exactly five completed public-trade minute buckets:
pressure t-5..t-2 plus reversal t-1. P48 V1.7 therefore keeps each asset fail-closed until all
five required completed minute buckets are actually present in the live publicTrade stream.

UI behaviour:

- Flow shows `TAPE n/5` while the asset is warming up;
- runtime summary shows Ready / Warm-up / Errors counts;
- the panel explicitly explains that five FULL live-tape minutes are required;
- any public-WS reconnect clears tape buckets and restarts this warm-up;
- wall-clock time without live publicTrade data can never move an asset to ready state.

This patch does not unlock Mainnet automatic Entry and does not change P46, Exit or Risk.
