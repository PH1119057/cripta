...
function Fail([string]$Message) {
    throw "EO1.1 install aborted: $Message"
}
...
Write-Host "EO1.1: verify payload hashes"
$hashRows = @()
...
try {
    ...
    Write-Host "EO1.1: create final-state code overlay"
    ...
    if (Test-Path -LiteralPath $reportsSource) {
        Write-Host "EO1.1: snapshot small report fixtures into overlay"
    ...
    Write-Host "EO1.1: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "EO1.1: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "EO1.1: overlay green; apply additive files"
    foreach ($file in $payloadFiles) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "EO1.1 installed successfully."
