...
def _load_trade_day(path: Path) -> TradeDay:
    ...
    return TradeDay(tuple(timestamps), tuple(prices))
...
def _combine_trade_days(first: TradeDay, second: TradeDay | None) -> TradeDay:
    if second is None:
        return first
    return TradeDay(first.timestamps + second.timestamps, first.prices + second.prices)
...
def run_flow_reversal_research(
    ...
    *,
    ...
    for day_index, day_key in enumerate(ordered_days, start=1):
        print(f"P31 raw-tape day {day_index}/{len(ordered_days)}: {day_key}")
        ...
        if day_key not in cache:
            cache[day_key] = _load_trade_day(current_path)
        ...
        if next_path is not None and next_day_key not in cache:
            cache[next_day_key] = _load_trade_day(next_path)
