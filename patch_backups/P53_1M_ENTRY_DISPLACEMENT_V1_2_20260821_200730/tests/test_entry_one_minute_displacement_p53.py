...
def test_one_minute_to_five_minute_equivalence_is_exact() -> None:
    ...
    assert result["ohlc_mismatches"] == 0
...
def test_trade_archive_missing_minute_fails_closed(tmp_path: Path) -> None:
    day = date(2026, 8, 1)
    start = datetime(2026, 8, 1, tzinfo=UTC)
    archive = tmp_path / "UNIUSDT2026-08-01.csv.gz"
    with gzip.open(archive, "wt", encoding="utf-8", newline="") as handle:
    ...
    try:
        ...
        assert "12:11:00+00:00" in str(exc)
    else:
        raise AssertionError("missing 1m candle must fail closed")
