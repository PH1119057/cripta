...
def _analyse_path(
    ...
    *,
    ...
    for index in range(start_index, end_index):
        ...
        move = _directional_move_pct(signal.direction, entry, tape.prices[index])
        moves.append((timestamp, move))
        if timestamp <= immediate_end_ts:
            mae_30 = move if mae_30 is None else min(mae_30, move)
            mfe_30 = move if mfe_30 is None else max(mfe_30, move)
        elapsed = timestamp - start_ts
        for threshold in config.favorable_thresholds_pct:
            key = _threshold_key(threshold)
            if favorable_hits[key] is None and move >= float(threshold):
                favorable_hits[key] = elapsed
        for threshold in config.adverse_thresholds_pct:
            key = _threshold_key(threshold)
            if adverse_hits[key] is None and move <= -float(threshold):
                adverse_hits[key] = elapsed

    def mae_before(target: Decimal) -> Decimal | None:
        target_seconds = favorable_hits[_threshold_key(target)]
        cutoff = end_ts if target_seconds is None else start_ts + target_seconds
        available = [move for timestamp, move in moves if timestamp <= cutoff]
        if not available:
            return None
        return Decimal(str(round(min(available), 8)))
