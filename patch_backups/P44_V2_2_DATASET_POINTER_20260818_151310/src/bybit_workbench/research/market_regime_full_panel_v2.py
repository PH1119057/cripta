    ...
    if provenance_path.is_file():
        ...
        if isinstance(sources, dict):
            ...
            if isinstance(p30_source, str) and p30_source:
                candidate = Path(p30_source) / "dataset" / "trade_5m.csv"
                if candidate.is_file():
                    return candidate, "legacy_materialization_p30"

    legacy_base = root / "reports" / "entry_research_v3"
    if legacy_base.is_dir():
        candidates = sorted(
            legacy_base.glob(f"{symbol}_*"),
        ...
        for candidate_dir in candidates:
            if not candidate_dir.is_dir():
                continue
            if not _metadata_matches_period(
                candidate_dir / "comparison.json",
                start=start,
                end=end,
            ):
                continue
            candidate = candidate_dir / "dataset" / "trade_5m.csv"
            if candidate.is_file():
                return candidate, "legacy_v3_search"
