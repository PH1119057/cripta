...
- boundary seed для первой минуты всего frozen evaluation period берётся **только из `open` первой frozen 5m свечи** этого же периода; это opening point в момент начала датасета, а не future-derived price;
