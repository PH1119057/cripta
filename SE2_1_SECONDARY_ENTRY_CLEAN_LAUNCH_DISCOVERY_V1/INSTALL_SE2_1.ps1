...
function Fail([string]$Message) {
    throw "SE2.1 install aborted: $Message"
}
...
Write-Host "SE2.1: verify payload hashes"
$hashRows = @()
...
try {
    ...
    Write-Host "SE2.1: create final-state code overlay"
    ...
    if (Test-Path -LiteralPath $reportsSource) {
        Write-Host "SE2.1: snapshot small report fixtures into overlay"
    ...
    Write-Host "SE2.1: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "SE2.1: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "SE2.1: overlay green; apply additive files"
    foreach ($file in $payloadFiles) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "SE2.1 installed successfully."
