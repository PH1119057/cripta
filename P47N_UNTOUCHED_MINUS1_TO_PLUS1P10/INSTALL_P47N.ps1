...
function Fail([string]$Message) {
    throw "P47N install aborted: $Message"
}
...
Write-Host "P47N: verify payload hashes"
$hashRows = @()
...
try {
    ...
    Write-Host "P47N: create code-only post-patch overlay"
    ...
    Write-Host "P47N: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "P47N: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "P47N: precheck green; apply real files"
    foreach ($file in $payloadFiles) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47N installed successfully."
