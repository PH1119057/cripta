...
function Fail([string]$Message) {
    throw "SE1.2 install aborted: $Message"
}
...
Write-Host "SE1.2: verify payload hashes"
$hashRows = @()
...
try {
    ...
    Write-Host "SE1.2: create final-state code overlay"
    ...
    Write-Host "SE1.2: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "SE1.2: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "SE1.2: overlay green; apply additive files"
    foreach ($file in $payloadFiles) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "SE1.2 installed successfully."
