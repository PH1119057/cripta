...
function Fail([string]$Message) {
    throw "P47K.1 install aborted: $Message"
}
...
Write-Host "P47K.1: verify payload hashes"
...
$tempRoot = Join-Path $env:TEMP ("cripta-p47k1-overlay-" + [guid]::NewGuid().ToString("N"))
$backupRoot = $null
...
try {
    ...
    Write-Host "P47K.1: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47K.1: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "P47K.1: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "P47K.1: precheck green; apply real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47K_1_" + $stamp)
    foreach ($file in Get-ChildItem -LiteralPath $payloadRoot -Recurse -File) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47K.1 installed successfully."
