...
try {
    Write-Host "============================================================="
    Write-Host "ZS1 V1.2 MYPY FIX - FAIL-CLOSED INSTALLER"
    ...
    Write-Host "ZS1 V1.2: verify frozen report prerequisites"
    $ExpectedSources = @{
    ...
    Write-Host "ZS1 V1.2: verify payload hashes"
    ...
    Write-Host "ZS1 V1.2: create final-state overlay"
    $Overlay = Join-Path $env:TEMP ("zs1_v12_overlay_" + [Guid]::NewGuid().ToString("N"))
    ...
    Write-Host "ZS1 V1.2: PowerShell 5.1 syntax precheck"
    ...
    Write-Host "ZS1 V1.2: py_compile changed Python"
    & $Python -m py_compile `
    ...
    try {
        Write-Host "ZS1 V1.2: Ruff final overlay src + tests"
        & $Ruff check src tests
        if ($LASTEXITCODE -ne 0) {
            Fail "Ruff src/tests overlay precheck failed"
        }

        Write-Host "ZS1 V1.2: mypy full overlay source"
        & $Mypy src
        if ($LASTEXITCODE -ne 0) {
            Fail "mypy overlay precheck failed"
        }

        Write-Host "ZS1 V1.2: targeted pytest"
    ...
    Write-Host "ZS1 V1.2: overlay precheck PASSED; project may now be changed"

    $Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $Root ("patch_backups\ZS1_V1_2_MYPY_FIX_" + $Stamp)
    ...
    Write-Host "ZS1 V1.2: install complete"
    ...
    Write-Error $_
    Write-Host "ZS1 V1.2 INSTALL FAILED."
