...
def _timing_summary(name: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    p25 = _percentile(delays, 0.25) if delays else None
    p75 = _percentile(delays, 0.75) if delays else None
    return {
        "name": name,
        "n": len(delays),
        "p25_minutes": (p25 / 60.0 if p25 is not None else None),
        "median_minutes": (statistics.median(delays) / 60.0 if delays else None),
        "p75_minutes": (p75 / 60.0 if p75 is not None else None),
