...
def test_resolve_frozen_5m_uses_legacy_materialization_source(tmp_path: Path) -> None:
    start = datetime(2026, 5, 18, tzinfo=UTC)
    end = datetime(2026, 8, 16, tzinfo=UTC)
    asset_root = (
        tmp_path
    ...
    legacy_p30 = tmp_path / "reports" / "entry_research_v3" / "UNIUSDT_legacy"
    legacy_price = legacy_p30 / "dataset" / "trade_5m.csv"
    legacy_price.parent.mkdir(parents=True)
    legacy_price.write_text("closed_at,close\n", encoding="utf-8")
    provenance = {
        "symbol": "UNIUSDT",
        "mode": "legacy_report_materialization",
        "heavy_datasets_copied": False,
        "sources": {"p30": str(legacy_p30)},
    }
    (asset_root / "legacy_materialization.json").write_text(
    ...
    assert source == "legacy_materialization_p30"
