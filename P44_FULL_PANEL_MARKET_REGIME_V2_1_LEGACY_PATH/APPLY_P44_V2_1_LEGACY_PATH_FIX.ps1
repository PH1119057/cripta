...
Write-Host "============================================================="
Write-Host "P44 V2.1 LEGACY 5M PATH FIX - PRECHECK"
...
$backupRoot = Join-Path $ProjectRoot "patch_backups\P44_V2_1_LEGACY_PATH_$stamp"
foreach ($item in $manifest) {
    $relative = ([string]$item.relative_path).Replace([char]'/', [IO.Path]::DirectorySeparatorChar)
    $source = Join-Path $PayloadRoot $relative
...
Write-Host ""
Write-Host "Expected UNI/LINK precheck source: legacy_materialization_p30 or legacy_v3_search"
Write-Host "============================================================="
