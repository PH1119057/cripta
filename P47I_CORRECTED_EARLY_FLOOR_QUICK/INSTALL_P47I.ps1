...
function Fail([string]$Message) {
    throw "P47I install aborted: $Message"
}
...
if (-not (Test-Path $python)) {
    Fail ".venv is missing; run scripts\setup_windows.ps1 first"
}
...
Write-Host "P47I: verify payload hashes"
...
$tempRoot = Join-Path $env:TEMP ("cripta-p47i-overlay-" + [guid]::NewGuid().ToString("N"))
$backupRoot = $null
...
try {
    ...
    Write-Host "P47I: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47I: PowerShell 5.1 syntax precheck"
    $psTokens = $null
    $psErrors = $null
    ...
    Write-Host "P47I: precheck green; now applying real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47I_EARLY_FLOOR_" + $stamp)
    foreach ($file in Get-ChildItem -LiteralPath $payloadRoot -Recurse -File) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47I installed successfully."
