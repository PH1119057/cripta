...
function Fail([string]$Message) {
    throw "P47J install aborted: $Message"
}
...
Write-Host "P47J: verify payload hashes"
...
$tempRoot = Join-Path $env:TEMP ("cripta-p47j-overlay-" + [guid]::NewGuid().ToString("N"))
$backupRoot = $null
...
try {
    ...
    Write-Host "P47J: create post-patch overlay"
    $exclude = @(
    ...
    Write-Host "P47J: PowerShell 5.1 syntax precheck"
    $tokens = $null
    $errors = $null
    ...
    try {
        ...
        Write-Host "P47J: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "P47J: precheck green; apply real files"
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupRoot = Join-Path $projectRoot ("patch_backups\P47J_" + $stamp)
    foreach ($file in Get-ChildItem -LiteralPath $payloadRoot -Recurse -File) {
        $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "P47J installed successfully."
