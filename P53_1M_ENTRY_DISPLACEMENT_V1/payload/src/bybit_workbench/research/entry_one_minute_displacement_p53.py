...
P53_VERSION = "P53_1M_ENTRY_DISPLACEMENT_V1"
CACHE_VERSION = "P53_1M_OHLC_CACHE_V1"
...
class P53Config:
    ...
    def __post_init__(self) -> None:
        if self.expected_signals != EXPECTED_SIGNALS:
            raise ValueError("P53 V1 is frozen to the 1063 Entry V1 ALL9 cohort")
        if self.horizon_hours != DEFAULT_HORIZON_HOURS:
            raise ValueError("P53 V1 deeper-entry availability horizon is frozen to 3 hours")
...
def _aggregate_trade_archive(
    ...
    *,
    ...
    if len(candles) != expected_rows:
        present = {item.opened_at for item in candles}
        missing = [
            expected_start + timedelta(minutes=index)
            for index in range(expected_rows)
            if expected_start + timedelta(minutes=index) not in present
        ]
        preview = ", ".join(item.isoformat() for item in missing[:10])
        raise ValueError(
            f"1m trade-derived candle grid is incomplete for {symbol} {day}: "
            f"rows={len(candles)} expected={expected_rows}; missing={preview}"
        )
...
def _cache_day(
    *,
    ...
    if cache_path.is_file() and meta_path.is_file() and archive_path.is_file():
        ...
        if compatible:
            return _read_candle_cache(cache_path, symbol=symbol)
...
def _load_one_minute_dataset(
    *,
    ...
    archive_by_day = _archive_map(dataset_dir, symbol)
...
def validate_five_minute_equivalence(
    ...
    return {"compared_5m_candles": compared, "ohlc_mismatches": 0}
...
def run_p53(
    ...
    *,
    ...
    for symbol_index, source in enumerate(sources, start=1):
        ...
        archive_by_day = _archive_map(source.dataset_dir, symbol)
