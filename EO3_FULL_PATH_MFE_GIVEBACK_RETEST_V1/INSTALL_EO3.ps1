...
function Fail([string]$Message) { throw "EO3 install aborted: $Message" }
...
Write-Host "EO3: verify exact EO1.2 + EO2.3 baseline"
...
Write-Host "EO3: verify payload hashes"
$hashRows = @()
...
try {
    ...
    Write-Host "EO3: create final-state code overlay"
    ...
    if (Test-Path -LiteralPath $reportsSource) {
        Write-Host "EO3: snapshot small report fixtures into overlay"
    ...
    Write-Host "EO3: PowerShell 5.1 syntax precheck"
    ...
    try {
        ...
        Write-Host "EO3: targeted pytest"
        ...
        Write-Host "EO3: full pytest"
        & $python -m pytest
        if ($LASTEXITCODE -ne 0) { Fail "full pytest failed" }
        Write-Host "EO3: headless smoke"
        ...
        Write-Host "EO3: GUI smoke (offscreen, replay/disarmed)"
    ...
    Write-Host "EO3: overlay green; apply new research files"
    try {
        foreach ($file in $payloadFiles) {
            $relative = $file.FullName.Substring($payloadRoot.Length).TrimStart("\")
    ...
    Write-Host "EO3: authoritative final Windows gate"
    ...
    Write-Host "EO3 installed successfully."
